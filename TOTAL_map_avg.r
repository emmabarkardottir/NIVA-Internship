...
# ICES AREA: Average F/FMSy_norm per stock (last 5 years), then average per area
...
# GFCM AREA (MEDBS): Average F/FMSy_norm per stock (last 5 years), then average per area
avg_per_stock_MEDBS <- species_data_MEDBS %>%
  filter(Year %in% tail(sort(unique(Year)), 5)) %>%
...
map_plot <- ggplot() +
  ...
  theme_minimal() +
  labs(title = "Fisheries Status Across European Seas, Mediterranean, and Black Sea")
