...
generate_plots <- function(ecoregion_id, output_folder = NULL) {
  ...
  combined_plot <- ggplot() +
    annotate("rect", xmin = min(species_area$Year), xmax = max(species_area$Year), ymin = 0.8, ymax = 1, fill = "blue", alpha = 0.7) +
    annotate("rect", xmin = min(species_area$Year), xmax = max(species_area$Year), ymin = 0.6, ymax = 0.8, fill = "green", alpha = 0.7) +
    annotate("rect", xmin = min(species_area$Year), xmax = max(species_area$Year), ymin = 0.4, ymax = 0.6, fill = "yellow", alpha = 0.8) +
    annotate("rect", xmin = min(species_area$Year), xmax = max(species_area$Year), ymin = 0.2, ymax = 0.4, fill = "orange", alpha = 0.8) +
    annotate("rect", xmin = min(species_area$Year), xmax = max(species_area$Year), ymin = 0, ymax = 0.2, fill = "red", alpha = 0.7) +
    geom_line(data = species_area, aes(x = Year, y = F.FMSy_flipped, group = StockKeyLabel), color = "#827e7ea1", size = 1) + 
    geom_point(data = species_area, aes(x = Year, y = F.FMSy_flipped, group = StockKeyLabel), color = "#827e7ea1", size = 1.5) +
    ...
    scale_y_continuous(breaks = seq(0, 1, by = 0.2)) +
    labs(title = paste("Flipped F/FMSy Over Time for all stocks in", ecoregion, "with Smoothed Average"),
         x = "Year", y = "Flipped F/FMSy (normalized)") +  
    theme_minimal()
